...
function loadfile (id, path, file) {
...
music.set( 'onended', function() { music.timeend = new Date();} );
    }
...
thea.href= 'javascript:loadfile('+i+', \''+nodos[i].path+'\',\''+nodos[i].file+'\');';
if (nodos[i].title == '')
thea.innerHTML = nodos[i].file;
else
   thea.innerHTML = nodos[i].artist + ' - '+nodos[i].title;
